...
if ($user["info"])
	print("<tr><td align=\"left\" colspan=\"2\" class=\"text\">" . format_comment($user["info"],true) . "</td></tr>\n");
}
else
{
...
if (get_user_class() >= $prfmanage_class && $user["class"] < get_user_class())
{
	...
	if(get_user_class() >= UC_MODERATOR)
		tr($lang_userdetails['row_show_bitbucket'], "<a href=bitbucket-list.php?mode=0&id=".$user['id'].">".$lang_userdetails['text_show_avatar']."</a>&nbsp;&nbsp;<a href=bitbucket-list.php?mode=1&id=".$user['id'].">".$lang_userdetails['text_show_attachments']."</a>", 1);
	...
	if (get_user_class() >= $cruprfmanage_class)
	{
		...
		tr($lang_userdetails['row_last_signin_time'], "<input type=\"datetime-local\" size=\"60\" name=\"last_signin_time\" value=\"" .htmlspecialchars(convertDateFormat($user[lastsignintime])) . "\" /><input type=\"hidden\"  name=\"ori_last_signin_time\" value=\"" .htmlspecialchars(convertDateFormat($user[lastsignintime])) . "\" />", 1);
