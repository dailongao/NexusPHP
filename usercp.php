...
// 资源对象
$res = get_current_user_resource()['user_control_panel'];
...
function submit() {
	global $lang_usercp;
	print("<tr><td class=\"rowhead\" valign=\"top\" align=\"right\"><div>".$lang_usercp['row_save_settings']."</div></td><td class=\"rowfollow\" valign=\"top\" align=left><div><input type=submit value=\"".$lang_usercp['submit_save_settings']."\"></div></td></tr>"."</form>");
}
		...
		switch ($action) {
			case "personal":
				if ($type == 'save') {
					...
					$updateset[] = "showonline = " . sqlesc($showonline);
...
<input type=radio name=gender" . ($CURUSER["gender"] == "Male" ? " checked" : "") . " value=Male>".$lang_usercp['radio_male']."<input type=radio name=gender" .  ($CURUSER["gender"] == "Female" ? " checked" : "") . " value=Female>".$lang_usercp['radio_female'],1);
  ...
  $text
	  ...
	  "\"><br />\n".$lang_usercp['text_avatar_note'].($enablebitbucket_main == 'yes' ? $lang_usercp['text_bitbucket_note'] : ""),1);
				...
				{
					tr_small($lang_usercp['row_showonline'],
				"<input type=radio name=showonline" . ($CURUSER["showonline"] == "yes" ? " checked" : "") . " value=yes>".$lang_usercp['radio_yes']."<input type=radio name=showonline" .  ($CURUSER["showonline"] == "no" ? " checked" : "") . " value=no>".$lang_usercp['radio_no'],1);
				}
			...
			case "tracker":
				...
				if ($type == 'save') {
					...
					if ($showextinfo['douban'] == 'yes'){
						if ($_POST["showdouban"] == 'yes')
							$showdouban = 'yes';
						else $showdouban = 'no';
						$updateset[] = "showdouban = " . sqlesc($showdouban);
					}
