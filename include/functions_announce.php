...
function check_cheater($userid, $torrentid, $uploaded, $downloaded, $anctime, $seeders=0, $leechers=0){
	...
	if ($uploaded > 1073741824 && $upspeed > (12582912/$cheaterdet_security)) //Uploaded more than 1 GB with uploading rate higher than 12 MByte/S (For Consertive level). This is likely cheating.
	{
	...
	foreach ($clients as $row_allowed_ua)
	{
		...
		if($row_allowed_ua['peer_id_pattern'] != '')
		{
			...
			if(preg_match($row_allowed_ua['peer_id_pattern'], $peer_id, $match_target))
			{
				...
				else // need match exactly if peer_id_pattern != ''
				{
					if($row_allowed_ua['peer_id_start'] === substr($peer_id, 0, strlen($row_allowed_ua['peer_id_start'])))
						$allowed_flag_peer_id = true;
					else
						$allowed_flag_peer_id = false;
				}
