...
/*
function formatUrl_s($url, $newWindow = false, $text = '', $linkClass = '')
{
	//format Url safe version
	global $BASEURL;
	if (!$text) {
		$text = $url;
	}
	return addTempCode("<a" . ($linkClass ? " class=\"$linkClass\"" : '') . " href=\"" . get_protocol_prefix() . "$BASEURL/$url\"" . ($newWindow == true ? " target=\"_blank\"" : "") . ">$text</a>");
}
*/
...
function format_urls($text, $newWindow = false) {
  $regex = "/((?:(?:https?|ftp|gopher|news|telnet|mms|rtsp):\/\/|www\.)[-a-z0-9+&@#\/%?=~_|!:,.;]*[-a-z0-9+&@#\/%=~_|])/ei";
  return preg_replace(
    $regex,
...
function format_comment($text, $strip_html = true, $xssclean = false, $newtab = false, $imageresizer = true, $image_max_width = 700, $enableimage = true, $enableflash = true, $imagenum = -1, $image_max_height = 0, $adid = 0) {
  ...
  /*
	 ...
	 */
  // Change original code to prevent XSS
  if ($adid) {
    $s = preg_replace("/\[url=((https?|ftp|gopher|news|telnet|mms|rtsp):\/\/[^()\[\]<>\s]+)\](.+?)\[\/url\]/ei", "formatAdUrl(" . $adid . " ,'\\1', '\\3', " . ($newtab == true ? 1 : 0) . ", 'faqlink')", $s);
    $s = preg_replace("/\[url=([^\[\s]+?)\](.+?)\[\/url\]/ei", "formatAdUrl(" . $adid . " ,'\\1', '\\2', " . ($newtab == true ? 1 : 0) . ", 'faqlink')", $s);
  } else {
    $s = preg_replace("/\[url=((https?|ftp|gopher|news|telnet|mms|rtsp):\/\/[^()\[\]<>\s]+)\](.+?)\[\/url\]/ei", "formatUrl('\\1', " . ($newtab == true ? 1 : 0) . ", '\\3', 'faqlink')", $s);
    $s = preg_replace("/\[url=([^\[\s]+?)\](.+?)\[\/url\]/ei", "formatUrl('\\1', " . ($newtab == true ? 1 : 0) . ", '\\2', 'faqlink')", $s);
  }

  // [url]http://www.example.com[/url]
  $s = preg_replace(
    "/\[url\]((https?|ftp|gopher|news|telnet|mms|rtsp):\/\/[^()\[\]<>\s]+)\[\/url\]/ei",
...
function is_valid_hours($hr) {
  return is_numeric($hr) && ($hr >= 24) && (floor($hr) == $hr) && ($hr <= 168);
}

function is_valid_protype($protype) {
  return is_numeric($protype) && (floor($protype) == $protype) && (($protype == 2) || ($protype == 4));
}
...
function begin_frame($caption = "", $center = false, $padding = 10, $width = "100%", $caption_center = "left", $text_pos = "center") {
  $tdextra = "";

  if ($center) {
    $tdextra .= " align=\"" . $text_pos . "\"";
  }

  print(($caption ? "<h2 align=\"" . $caption_center . "\">" . $caption . "</h2>" : "") . "<table width=\"" . $width . "\" border=\"1\" cellspacing=\"0\" cellpadding=\"" . $padding . "\">" . "<tr><td class=\"text\" $tdextra><div>\n");
}
  ...
  ?>
    ...
    <script type="text/javascript">
        ...
        function cstat() {
            ...
            document.<?= $form ?>.tagcount.value = "Close last, Open " + c;
        }
        ...
        function closeall() {
            ...
            document.<?= $form ?>.tagcount.value = "Close last, Open 0";
            bbtags = new Array();
    ...
    <table width="100%" cellspacing="0" cellpadding="5" border="0">
        <tr>
            <td align="left" colspan="2">
                <table cellspacing="1" cellpadding="2" border="0">
                    <tr>
                        <td class="embedded">
                            <input style="font-weight: bold; font-size: 11px; margin-right: 3px" type="button" name="b"
                                   value="B" onclick="javascript: simpletag('b')"/>
                        </td>
                        <td class="embedded">
                            <input class="codebuttons" style="font-style: italic; font-size: 11px; margin-right: 3px"
                                   type="button" name="i" value="I" onclick="javascript: simpletag('i')"/>
                        </td>
                        <td class="embedded">
                            <input class="codebuttons"
                                   style="text-decoration: underline; font-size: 11px; margin-right: 3px" type="button"
                                   name="u" value="U" onclick="javascript: simpletag('u')"/>
                        </td>
                      <?php
                      ...
                      ?>
                        <td class="embedded">
                            <input class="codebuttons" style="font-size: 11px; margin-right: 3px" type="button"
                                   name="quote" value="QUOTE" onclick="javascript: simpletag('quote')"/>
                        </td>
                        <td class="embedded">
                            <input style="font-size: 11px; margin-right: 3px" type="button"
                                   onclick='javascript:closeall();' name='tagcount' value="Close all tags"/>
                        </td>
                        <td class="embedded">
                            <select class="med codebuttons" style="margin-right: 3px" name='color'
                                    onchange="alterfont(this.options[this.selectedIndex].value, 'color')">
                                <option value='0'>--- <?= $lang_functions['select_color'] ?> ---</option>
                        ...
                        <td class="embedded">
                            <select class="med codebuttons" name='font'
                                    onchange="alterfont(this.options[this.selectedIndex].value, 'font')">
                                <option value="0">--- <?= $lang_functions['select_font'] ?> ---</option>
                        ...
                        <td class="embedded">
                            <select class="med codebuttons" name='size'
                                    onchange="alterfont(this.options[this.selectedIndex].value, 'size')">
                                <option value="0">--- <?= $lang_functions['select_size'] ?> ---</option>
        ...
        ?>
          <tr>
              <td colspan="2" valign="middle">
                  <iframe src="<?= get_protocol_prefix() . $BASEURL ?>/attachment.php" width="100%" height="24"
      ...
      ?>
        ...
        <td align="center" width="99%">
            ...
            <br/>
            <a href="javascript:winop();"><?= $lang_functions['text_more_smilies'] ?></a>
        </td>
        </tr>
    </table>
  <?php
...
function begin_compose($title = "", $type = "new", $body = "", $hassubject = true, $subject = "", $maxsubjectlength = 100, $hasmodechoose = false, $casinomode = false) {
  global $lang_functions;
  if ($title) {
  ...
  if ($hassubject) {
    print("<tr><td class=\"rowhead\">" . $lang_functions['row_subject'] . "</td>" .
      "<td class=\"rowfollow\" align=\"left\"><input type=\"text\" style=\"width: 650px;\" name=\"subject\" maxlength=\"" . $maxsubjectlength . "\" value=\"" . $subject . "\" />");
    if ($hasmodechoose) {
      print("&nbsp;&nbsp;<input type=\"checkbox\" name=\"casinomode\"" . ($casinomode ? " checked " : " ") . ">" . $lang_functions['row_casinoenable'] . "</input>");
    }
    print("</td></tr>\n");
  }
...
function get_external_tr($url = "", $ext = "imdb") {
  global $lang_functions;
  global $showextinfo;

  if ($showextinfo['imdb'] == 'yes' && $ext == "imdb") {
    $imdbNumber = parse_imdb_id($url);
    tr($lang_functions['row_imdb_url'], "<input type=\"text\" style=\"width: 650px;\" name=\"url\" value=\"" . build_imdb_url($imdbNumber) . "\" /><br /><font class=\"medium\">" . $lang_functions['text_imdb_url_note'] . "</font>", 1);
  } else {
    if ($showextinfo['douban'] == 'yes' && $ext == "douban") {
      $doubanNumber = parse_douban_id($url);
      tr($lang_functions['row_douban_url'], "<input type=\"text\" style=\"width: 650px;\" name=\"douban_url\" value=\"" . ($doubanNumber ? $url : "") . "\" /><br /><font class=\"medium\">" . $lang_functions['text_douban_url_note'] . "</font>", 1);
    }
  }
}
...
function parse_douban_type($url) {
  if (strpos($url, 'movie.douban.com') != FALSE) {
    return "movie";
  } else {
    if (strpos($url, 'music.douban.com') != FALSE) {
      return "music";
    } else {
      return "movie";
    }
...
function build_douban_url($douban_id, $douban_type) {
  if ($douban_id == "") {
    return "";
  } else {
    return "https://" . $douban_type . ".douban.com/subject/" . $douban_id . "/";
  }
}
...
function get_torrent_2_user_value($user_snatched_arr) {
  ...
  } else    // torrent already deleted, half blind guess, be conservative
  {
    ...
    if ($user_snatched_arr['finished'] == 'no' && $user_snatched_arr['uploaded'] > 0 && $user_snatched_arr['downloaded'] == 0)    // possibly owner
    {
      ...
      if ($user_snatched_arr['downloaded'] > 0)    // possibly leecher
      {
    ...
    if (count($addresses) > 0) {
      foreach ($addresses as $email) {
        ...
        if (preg_match('/^[^\\s]+@[^\\s]+$/', $email)) {
          // specific regular expression to match email address
          // e.g.: 3[0-9]{9}@zju.edu.cn
          $email = preg_replace('/@/', '[@\\.]', $email);
          if (preg_match('/^' . $email . '$/', $newEmail)) {
            return true;
          }
...
function sent_mail($to, $fromname, $fromemail, $subject, $body, $type = "confirmation", $showmsg = true, $multiple = false, $multiplemail = '', $hdr_encoding = 'UTF-8', $specialcase = '') {
  global $lang_functions;
  global $rootpath, $SITENAME, $SITEEMAIL, $smtptype, $smtp, $smtp_host, $smtp_port, $smtp_from, $smtpaddress, $smtpport, $smtpfrom, $accountname, $accountpassword, $eYouAccount, $eYouAccountDomain, $eYouAccountPass, $eYouUrl;
  ...
  if ($smtptype == 'default') {
    ...
    $mail->close();
  } elseif ($smtptype == 'eYou') {        // for zju emial system only

    if (stristr($to, '@zju.edu.cn') !== FALSE) {
      require_once($rootpath . 'include/smtp/smtp.lib.php');
      $mail = new smtp($hdr_encoding, 'eYou');
      $mail->debug(false);
      $mail->open($smtpaddress, $smtpport);
      ...
      require_once($rootpath . 'include/smtp/eYou.php');
      $mail_eYou = new eYou();
      $mail_eYou->set_account($eYouAccount, $eYouAccountDomain, $eYouAccountPass, $eYouUrl);
      $mail_eYou->set_message($to, $subject, $body);
...
function new_mysqli() {
  ...
  // MYSQLi 链接
  $result = new mysqli($mysql_host, $mysql_user, $mysql_pass, $mysql_db);
  // 设置字符集
...
function get_user_row($id) {
  ...
  static $neededColumns = array('id', 'noad', 'class', 'enabled', 'privacy', 'avatar', 'signature', 'uploaded', 'downloaded', 'last_access', 'username', 'donor', 'leechwarn', 'warned', 'title', 'showonline');
  if ($id == $CURUSER['id']) {
    $row = array();
...
function tr($x, $y, $noesc = 0, $relation = '') {
  ...
  print("<tr" . ($relation ? " relation = \"$relation\"" : "") . "><td class=\"rowhead nowrap\" valign=\"top\" align=\"right\"><div>$x</div></td><td class=\"rowfollow\" valign=\"top\" align=\"left\"><div>" . $a . "</div></td></tr>\n");
}
...
function tr_small($x, $y, $noesc = 0, $relation = '') {
  ...
  print("<tr" . ($relation ? " relation = \"$relation\"" : "") . "><td width=\"1%\" class=\"rowhead nowrap\" valign=\"top\" align=\"right\"><div>" . $x . "</div></td><td width=\"99%\" class=\"rowfollow\" valign=\"top\" align=\"left\"><div>" . $a . "</div></td></tr>\n");
}
...
function menu($selected = "home") {
  ...
  global $enableoffer, $enablespecial, $enableextforum, $extforumurl, $where_tweak, $offermanage_class;
  ...
  if ($enableoffer == 'yes') {
    ...
    if (get_user_class() >= $offermanage_class) {
      $res = sql_query("SELECT offers.allowed FROM offers");
      $count = mysql_num_rows($res);
      if ($count > 0) {
        $tail = '(' . $count . ')';
      }
    }
    print("<li" . ($selected == "offers" ? " class=\"selected\"" : "") . "><a href=\"offers.php\">" . $lang_functions['text_offers'] . $tail . "</a></li>");
  }
...
{
  global $lang_functions;
  global $CURUSER, $CURLANGDIR, $USERUPDATESET, $iplog1, $oldip, $SITE_ONLINE, $FUNDS, $SITENAME, $SLOGAN, $logo_main, $BASEURL, $offlinemsg, $showversion, $enabledonation, $staffmem_class, $titlekeywords_tweak, $metakeywords_tweak, $metadescription_tweak, $cssdate_tweak, $deletenotransfertwo_account, $neverdelete_account, $iniupload_main, $casino_tweak, $casino_class;
  global $tstart;
  global $Cache;
  ...
  ?>
    ...
    <head>
      ...
      if ($metakeywords_tweak) {
        ?>
          <meta name="keywords" content="<?= htmlspecialchars($metakeywords_tweak) ?>"/>
        <?php
      }
      if ($metadescription_tweak) {
        ?>
          <meta name="description" content="<?= htmlspecialchars($metadescription_tweak) ?>"/>
        <?php
      }
      ?>
      ...
      ?>
        ...
        <link rel="search" type="application/opensearchdescription+xml" title="<?= $SITENAME ?> Torrents"
              href="opensearch.php"/>
        ...
        <link rel="stylesheet" href="<?= get_forum_pic_folder() . "/forumsprites.css" . $cssupdatedate ?>"
              type="text/css"/>
        ...
        <link rel="stylesheet" href="styles/jquery.custom.css<?= $cssupdatedate ?>" type="text/css"/>
      <?php
      if ($CURUSER) {
        $caticonrow = get_category_icon_row($CURUSER['caticon']);
        if ($caticonrow['cssfile']) {
          ?>
            <link rel="stylesheet" href="<?= htmlspecialchars($caticonrow['cssfile']) . $cssupdatedate ?>"
                  type="text/css"/>
          <?php
        }
      }
      ?>
        ...
        <script type="text/javascript" src="parse-torrent.min.js"></script>
        <script type="text/javascript" src="jquery.min.js<?= $cssupdatedate ?>"></script>
        <script type="text/javascript" src="jquery.ui.min.js<?= $cssupdatedate ?>"></script>
        <script type="text/javascript" src="jquery.boxy.js<?= $cssupdatedate ?>"></script>
        <script type="text/javascript" src="jquery.scripts.js<?= $cssupdatedate ?>"></script>
    </head>
      ...
      if (mysql_num_rows($result) > 0) {
        ...
        ?>
          <div style="position: absolute; bottom: 20px; right: 20px;">
              <a href="easteregg.php?key=<?= $eggkey; ?>">
        ...
        <tr>
            <td class="clear">
              ...
              if ($logo_main == "") {
                ...
                ?>
                  <div class="logo_img">
                      <img src="<?= $logo_main ?>" alt="<?= htmlspecialchars($SITENAME) ?>"
                           title="<?= htmlspecialchars($SITENAME) ?> - <?= htmlspecialchars($SLOGAN) ?>"/>
                  </div>
                <?php
              }
              ?>
            ...
            <td class="clear nowrap" align="right" valign="middle">
              ...
              if ($enabledonation == 'yes') { ?>
                  <!-- <a href="donate.php">
						<img src="<?= get_forum_pic_folder() ?>/donate.gif" alt="Make a donation" style="margin-left: 5px; margin-top: 50px;" />
					</a> -->
                <?php
              }
              ?>
    ...
    <table class="mainouter" width="1060" cellspacing="0" cellpadding="5" align="center">
        <tr>
            <td id="nav_block" class="text" align="center">
              <?php if (!$CURUSER) { ?>
                  <a href="login.php">
                      <font class="big"><b><?= $lang_functions['text_login'] ?></b></font>
                  </a>/ <a href="signup.php">
                      <font class="big"><b><?= $lang_functions['text_signup'] ?></b></font>
                  </a>
              <?php } else {
              begin_main_frame();
              menu();
                            ...
                            <table width="100%" cellspacing="0" cellpadding="0" border="0">
                                <tr>
                                    <td class="bottom" align="left"><span
                                                ...
                                                <a href="casino.php#tabs-2"><?= $lang_functions['text_casino'] ?></a>]<?php } ?>[<a
                                                    ...
                                                    href="invite.php?id=<?= $CURUSER['id'] ?>"><?= $lang_functions['text_send'] ?></a>]: <?= $CURUSER['invites'] ?><br/>
														...
														<font class="color_ratio"><?= $lang_functions['text_ratio'] ?></font><?= $ratio ?> <font
                                                    class='color_uploaded'><?= $lang_functions['text_uploaded'] ?></font><?= mksize($CURUSER['uploaded']) ?><font
                                                    class='color_downloaded'> <?= $lang_functions['text_downloaded'] ?></font> <?= mksize($CURUSER['downloaded']) ?> <font
                                                    class='color_active'><?= $lang_functions['text_active_torrents'] ?></font>
														<img class="arrowup" alt="Torrents seeding"
                                                             title="<?= $lang_functions['title_torrents_seeding'] ?>"
                                                             src="pic/trans.gif"/><?= $activeseed ?>
														<img class="arrowdown" alt="Torrents leeching"
                                                             title="<?= $lang_functions['title_torrents_leeching'] ?>"
                                                             src="pic/trans.gif"/><?= $activeleech ?>&nbsp;&nbsp;<font
                                                    class='color_connectable'><?= $lang_functions['text_connectable'] ?></font><?= $connectable ?> <?= maxslots(); ?>
												</span></td>
                                    ...
                                    <td class="bottom" align="right"><span class="medium">
                                                    ...
                                                    if ($enabledonation) {
                                                      ?> [<a href="donate.php"
                                                             class="altlink_green"><?= $lang_functions['text_donate'] ?></a>] <?
                                                    }
                                                    ...
                                                    if (get_date_diff(new DateTime("now"), new DateTime($CURUSER["lastsignintime"])) == 0) {
                                                      ...
                                                      ?> [<a href="signin.php"
                                                             class="faqlink"><?= $lang_functions['text_sign_in'] ?></a>] <?
                                                    }
                                                    ?>
        ...
        <tr>
            <td id="outer" align="center" class="outer" style="padding-top: 20px; padding-bottom: 20px">
               ...
               * 获得语言 ID 对应的语言名称。
               */
              function get_sublang_name($lang_id) {

                $sql = new mysqli();

                $query = $sql->prepare("SELECT `lang_name` FROM `language` WHERE `sub_lang` = 1 AND `id` = ?");
                $query->bind_param("i", $lang_id);
                $query->execute();

                $query->bind($lang_name);
              ...
              function deletetorrent($id, $name, $is_anonymous, $rt, $reason, $deletesubs) {
                ...
                if ($deletesubs) {
                  ...
                  $sql = new_mysqli();
                  $query = $sql->prepare("SELECT `subs`.`id`, `ext`, `uppedby`, `title`, `anonymous`, `lang_name` FROM `subs` JOIN `language` ON `language`.`id` = `subs`.`lang_id` WHERE `torrent_id` = ?");
                  $query->bind_param("i", $id);
                  $query->execute() or sqlerr(__FILE__, __LINE__);

                  $query->bind_result($sub_id, $sub_ext, $sub_uppedby, $sub_title, $sub_anonymous, $sub_lang_name);

                  // 逐个删除字幕
                  while ($query->fetch()) {

                    // 字幕完整名称。
                    $sub_name = MessageFormatter::formatMessage("", "{0} {1}.{2}", array($sub_lang_name, $sub_title, $sub_ext));
                    ...
                    // 删除数据库数据
                    $q2 = $sql2->prepare("DELETE FROM `subs` WHERE id = ?");
                    $q2->bind_param("i", $sub_id);
                    $q2->execute() or sqlerr(__FILE__, __LINE__);

                    $sql2->close();

                    // 删除文件
                    unlink("$SUBSPATH/$id/$sub_id.$sub_ext");
                ...
                $sql = new_mysqli();
                // 种子数据库删除。
                $query = $sql->prepare("DELETE FROM `torrents` WHERE `id` = ?");
                $query->bind_param("i", $id);
                $query->execute();


                $query = $sql->prepare("DELETE FROM `snatched` WHERE `torrentid` = ?");
                $query->bind_param("i", $id);
                $query->execute();
              ...
              ?>
                <table class="torrents" cellspacing="0" cellpadding="5" width="100%">
                    <tr>
                      ...
                      ?>
                        <td class="colhead" style="padding: 0px"><?= $lang_functions['col_type'] ?></td>
                        <td class="colhead"><a
                                    href="?<?= $oldlink ?>sort=1&amp;type=<?= $link[1] ?>"><?= $lang_functions['col_name'] ?></a>
                        </td>
                      <?php
                      ...
                      if ($CURUSER['showcomnum'] != 'no') { ?>
                          <td class="colhead"><a href="?<?= $oldlink ?>sort=3&amp;type=<?= $link[3] ?>">
                                  <img class="comments" src="pic/trans.gif" alt="comments"
                                       title="<?= $lang_functions['title_number_of_comments'] ?>"/></a></td>
                      <?php } ?>

                        <td class="colhead"><a href="?<?= $oldlink ?>sort=4&amp;type=<?= $link[4] ?>">
                                <img class="time" src="pic/trans.gif" alt="time"
                                     title="<?= ($CURUSER['timetype'] != 'timealive' ? $lang_functions['title_time_added'] : $lang_functions['title_time_alive']) ?>"/></a>
                        </td>
                        <td class="colhead"><a href="?<?= $oldlink ?>sort=5&amp;type=<?= $link[5] ?>">
                                <img class="size" src="pic/trans.gif" alt="size"
                                     title="<?= $lang_functions['title_size'] ?>"/></a></td>
                        <td class="colhead"><a href="?<?= $oldlink ?>sort=7&amp;type=<?= $link[7] ?>">
                                <img class="seeders" src="pic/trans.gif" alt="seeders"
                                     title="<?= $lang_functions['title_number_of_seeders'] ?>"/></a></td>
                        <td class="colhead"><a href="?<?= $oldlink ?>sort=8&amp;type=<?= $link[8] ?>">
                                <img class="leechers" src="pic/trans.gif" alt="leechers"
                                     title="<?= $lang_functions['title_number_of_leechers'] ?>"/></a></td>
                        <td class="colhead"><a href="?<?= $oldlink ?>sort=6&amp;type=<?= $link[6] ?>">
                                <img class="snatched" src="pic/trans.gif" alt="snatched"
                                     title="<?= $lang_functions['title_number_of_snatched'] ?>"/></a></td>
                        <td class="colhead"><a
                                    href="?<?= $oldlink ?>sort=9&amp;type=<?= $link[9] ?>"><?= $lang_functions['col_uploader'] ?></a>
                        </td>
                      <?php
                      if (get_user_class() >= $torrentmanage_class) { ?>
                          <td class="colhead"><?= $lang_functions['col_action'] ?></td>
                      <?php } ?>
                    </tr>
  <?php
  $caticonrow = get_category_icon_row($CURUSER['caticon']);
  ...
  while ($row = mysql_fetch_assoc($res)) {
    ...
    switch ($tooltiptype) {
      ...
      case 'minordouban':
      {
        if ($showextinfo['douban'] == 'yes' && $row["douban_url"]) {
          $url = $row['douban_url'];
          $cache = $row['douban_cache_stamp'];
          $type = 'minor';
          $has_tooltip = true;
        }
        break;
    ...
    if (!$has_tooltip) {
      ...
      if ($tooltiptype == 'minorimdb' || $tooltiptype == 'medianimdb') {
        ...
        if ($tooltiptype == 'minordouban' || $tooltiptype == 'mediandouban') {
          $mouseovertorrent = "onmouseover=\"get_douban_ajax('" . $torrent_tooltip[$counter]['id'] . "','" . $url . "','" . $cache . "','" . $type . "'); domTT_activate(this, event, 'content', document.getElementById('" . $torrent_tooltip[$counter]['id'] . "'), 'trail', false, 'delay',600,'lifetime',6000,'fade','both','styleClass','niceTitle', 'fadeMax',87, 'maxWidth', 500);\"";
        }
      }
    }
    ...
    if (mysql_num_rows($r) > 0) {
      /*print("<td width=30 valign=\"middle\" align=\"right\" style=\"border:0px;\"><img src=pic/sub.png border=\"0\" style=\"padding-bottom:2px;\" name=subicon_102440 class=\"subicon\" /></td>");*/
      print("<td valign=\"middle\" align=\"right\" style=\"border:0px;\">");
      //$markflag=array();
      while ($row_r = mysql_fetch_array($r)) {
        //if(!$markflag[$row_r["lang_name"]]){
        print("<a href=\"downloadsubs.php?torrentid=" . $row_r[torrent_id] . "&subid=" . $row_r[id] . "\">");
        print("<img border=\"0\" src=\"pic/flag/" . $row_r["flagpic"] . "\" alt=\"" . $row_r["lang_name"] . "\" title=\"" . $row_r["lang_name"] . "\" style=\"width: 16px; height:10px;\" />&nbsp;");
        print("</a>");
        //$markflag[$row_r["lang_name"]] = 1;
        //}
      }
      print("</td>");
    }
    ...
    //show status in torrent table
    //because of the hugh query number, so stop it for while
    /*
		$seedingstatus = ""; $seedingexp = "";
		$stacheck = sql_query("SELECT * FROM peers WHERE userid = ".$CURUSER['id']." AND torrent = $id AND seeder='yes'") or sqlerr();
		$stacount = mysql_num_rows($stacheck);
		if($stacount > 0){
		$seedingstatus = " seedhlc_current";
		$seedingexp = $lang_functions['text_seeding'];
		}
		$leechingstatus = ""; $leechingexp = "";
		$stacheck = sql_query("SELECT * FROM peers WHERE userid = ".$CURUSER['id']." AND torrent = $id AND seeder='no'") or sqlerr();
		$stacount = mysql_num_rows($stacheck);
		if($stacount > 0){
		$leechingstatus = " leechhlc_current";
		$leechingexp = $lang_functions['text_leeching'];
		}
		$completestatus = ""; $completeexp = "";
		$stacheck = sql_query("SELECT * FROM snatched WHERE userid = ".$CURUSER['id']." AND torrentid = $id AND finished='yes'") or sqlerr();
		$stacount = mysql_num_rows($stacheck);
		if($stacount > 0){
		$completestatus = " snatchhlc_finish";
		$completeexp = $lang_functions['text_complete'];
		}
		 */
    ...
    if ($row["seeders"]) {
      ...
      print("<td class=\"rowfollow" . $seedingstatus . "\" title=\"" . $seedingexp . "\" align=\"center\"><b><a href=\"details.php?id=" . $id . "&amp;hit=1&amp;dllist=1#seeders\">" . ($ratiocolor ? "<font color=\"" .
          $ratiocolor . "\">" . number_format($row["seeders"]) . "</font>" : number_format($row["seeders"])) . "</a></b></td>\n");
    } else {
      print("<td class=\"rowfollow" . $seedingstatus . "\" title=\"" . $seedingexp . "\"><span class=\"" . linkcolor($row["seeders"]) . "\">" . number_format($row["seeders"]) . "</span></td>\n");
    }

    if ($row["leechers"]) {
      print("<td class=\"rowfollow" . $leechingstatus . "\" title=\"" . $leechingexp . "\"><b><a href=\"details.php?id=" . $id . "&amp;hit=1&amp;dllist=1#leechers\">" .
        number_format($row["leechers"]) . "</a></b></td>\n");
    } else {
      print("<td class=\"rowfollow" . $leechingstatus . "\" title=\"" . $leechingexp . "\">0</td>\n");
    }

    if ($row["times_completed"] >= 1) {
      print("<td class=\"rowfollow" . $completestatus . "\" title=\"" . $completeexp . "\"><a href=\"viewsnatches.php?id=" . $row[id] . "\"><b>" . number_format($row["times_completed"]) . "</b></a></td>\n");
    } else {
      print("<td class=\"rowfollow" . $completestatus . "\" title=\"" . $completeexp . "\">" . number_format($row["times_completed"]) . "</td>\n");
    }
  ...
  print("<p align=\"center\"> " . $lang_functions['text_torrents_with_tags_note'] . "</p>\n");
...
function getdouban($douban_id, $douban_type, $douban_cache_stamp, $mode = 'minor') {
  global $lang_functions;
  global $showextinfo;
  ...
  switch ($douban->cache_state()) {
    ...
    case "1": //cache is ready
    {
      if ($douban_type == "movie") {
        ...
        $temp = "";
        for ($i = 0; $i < count($douban->country); $i++) {
          $temp .= $douban->country[$i] . ", ";
        }
        $country = rtrim(trim($temp), ",");

        $temp = "";
        for ($i = 0; $i < count($douban->language); $i++) {
          $temp .= $douban->language[$i] . ", ";
        }
        $language = rtrim(trim($temp), ",");

        $runtime = $douban->duration[0];
        $director = $douban->director;

        $temp = "";
        for ($i = 0; $i < count($douban->cast); $i++) //get names of first three casts
        {
          if ($i > 2) {
            break;
          }
        ...
        $cast = rtrim(trim($temp), ",");

        $genre = $douban->genre[0] . (count($douban->genre) > 1 ? ", " . $douban->genre[1] : ""); //get first two genres;
        $summary = $douban->summary;
        $votes = $douban->raters;
        if ($votes > 9) {
          $rating = $douban->rating;
        } else {
          $rating = $lang_functions['text_no_enough_votes_yet'];
        }

        switch ($mode) {
          case 'minor':
          {
            $autodata = "<font class=\"big\"><b>" . $title . "</b></font> (" . $year . ") <br /><strong><font color=\"DarkRed\">" . $lang_functions['text_douban'] . ": </font></strong>" . ($votes > 10 ? "<b>" . $rating . "</b>/10 (" . $votes . $lang_functions['text_votes'] . ")" : $lang_functions['text_no_enough_votes_yet']) . " <strong><font color=\"DarkRed\">" . $lang_functions['text_country'] . ": </font></strong>" . $country . " <strong><font color=\"DarkRed\">" . $lang_functions['text_genres'] . ": </font></strong>" . $genre . "<br />" . "<strong><font color=\"DarkRed\">" . $lang_functions['text_director'] . ": </font></strong>" . $director . "<strong><font color=\"DarkRed\"> " . $lang_functions['text_starring'] . ": </font></strong>" . $cast . "<br />";
            break;
          }
          case 'median':
          {
            if ($douban->image_state() != FALSE) {
              $smallth = "<img src=\"" . $douban->image_path . "\" width=\"105\" alt=\"poster\" />";
            } else {
              $smallth = "";
            }

            $summary = mb_substr($summary, 0, 120, "UTF-8") . (mb_strlen($summary, "UTF-8") > 120 ? " ..." : "");
            $summary = "<font class=\"small\">" . $summary . "</font>";
            ...
            $autodata = "<table style=\"background-color: transparent;\" border=\"0\" cellspacing=\"0\" cellpadding=\"3\">
              ...
              . "<td class=\"clear\" valign=\"top\" align=\"left\">
												<table style=\"background-color: transparent;\" border=\"0\" cellspacing=\"0\" cellpadding=\"3\" width=\"350\">
												<tr><td class=\"clear\" colspan=\"2\"><img class=\"douban\" src=\"pic/trans.gif\" alt=\"douban\" /> <font class=\"big\"><b>" . $title . "</b></font> (" . $year . ") </td></tr>
												<tr><td class=\"clear\"><strong><font color=\"DarkRed\">" . $lang_functions['text_douban'] . ": </font></strong>" . ($votes > 10 ? "<b>" . $rating . "</b>/10 (" . $votes . $lang_functions['text_votes'] . ")" : $lang_functions['text_no_enough_votes_yet']) . "</td>
												" . ($runtime ? "<td class=\"clear\"><strong><font color=\"DarkRed\">" . $lang_functions['text_runtime'] . ": </font></strong>" . $runtime . "</td>" : "<td class=\"clear\"></td>") . "</tr>
												<tr><td class=\"clear\"><strong><font color=\"DarkRed\">" . $lang_functions['text_country'] . ": </font></strong>" . $country . "</td>
												" . ($language ? "<td class=\"clear\"><strong><font color=\"DarkRed\">" . $lang_functions['text_language'] . ": </font></strong>" . $language . "</td>" : "<td class=\"clear\"></td>") . "</tr>
												<tr><td class=\"clear\">" . "<strong><font color=\"DarkRed\">" . $lang_functions['text_director'] . ": </font></strong>" . $director . "</td>
												<td class=\"clear\"><strong><font color=\"DarkRed\">" . $lang_functions['text_genres'] . ": </font></strong>" . $genre . "</td></tr>
												<tr><td class=\"clear\" colspan=\"2\"><strong><font color=\"DarkRed\">" . $lang_functions['text_starring'] . ": </font></strong>" . $cast . "</td></tr>
												" . ($summary ? "<tr><td class=\"clear\" colspan=\"2\">" . $summary . "</td></tr>" : "") . "
												</table>
												</td>
												</table>";
            break;
        ...
        if ($douban_type == "music") {
          ...
          $temp = "";
          for ($i = 0; $i < count($douban->singer); $i++) {
            $temp .= $douban->singer[$i] . ", ";
          }
          $singer = rtrim(trim($temp), ",");

          $temp = "";
          for ($i = 0; $i < count($douban->pubdate); $i++) {
            $temp .= $douban->pubdate[$i] . ", ";
          }
          $pubdate = rtrim(trim($temp), ",");

          $temp = "";
          for ($i = 0; $i < count($douban->media); $i++) {
            $temp .= $douban->media[$i] . ", ";
          }
          $media = rtrim(trim($temp), ",");

          $temp = "";
          for ($i = 0; $i < count($douban->tag) && $i < 2; $i++) {
            $temp .= $douban->tag[$i][0] . ", ";
          }
          $tag = rtrim(trim($temp), ",");

          $temp = "";
          for ($i = 0; $i < count($douban->publisher); $i++) {
            $temp .= $douban->publisher[$i] . ", ";
          }
          $publisher = rtrim(trim($temp), ",");
          ...
          switch ($mode) {
            case 'minor':
            {
              $autodata = "<font class=\"big\"><b>" . $title . "</b></font><br /><strong><font color=\"DarkRed\">" . $lang_functions['text_douban'] . ": </font></strong>" . ($votes > 10 ? "<b>" . $rating . "</b>/10 (" . $votes . $lang_functions['text_votes'] . ")" : $lang_functions['text_no_enough_votes_yet']) . " <strong><font color=\"DarkRed\">" . $lang_functions['text_pubdate'] . ": </font></strong>" . $pubdate . " <strong><font color=\"DarkRed\">" . $lang_functions['text_media'] . ": </font></strong>" . $media . "<br />" . "<strong><font color=\"DarkRed\">" . $lang_functions['text_singer'] . ": </font></strong>" . $singer . "<strong><font color=\"DarkRed\"> " . $lang_functions['text_type'] . ": </font></strong>" . $type . "<br />";
              break;
            }
            case 'median':
            {
              ...
              $autodata = "<table style=\"background-color: transparent;\" border=\"0\" cellspacing=\"0\" cellpadding=\"3\">
                ...
                . "<td class=\"clear\" valign=\"top\" align=\"left\">
												<table style=\"background-color: transparent;\" border=\"0\" cellspacing=\"0\" cellpadding=\"3\" width=\"350\">
												<tr><td class=\"clear\" colspan=\"2\"><img class=\"douban\" src=\"pic/trans.gif\" alt=\"douban\" /> <font class=\"big\"><b>" . $title . "</b></font></td></tr>
												<tr><td class=\"clear\"><strong><font color=\"DarkRed\">" . $lang_functions['text_douban'] . ": </font></strong>" . ($votes > 10 ? "<b>" . $rating . "</b>/10 (" . $votes . $lang_functions['text_votes'] . ")" : $lang_functions['text_no_enough_votes_yet']) . "</td>
												" . "<td class=\"clear\"><strong><font color=\"DarkRed\">" . $lang_functions['text_pubdate'] . ": </font></strong>" . $pubdate . "</td>" . "</tr>
												<tr><td class=\"clear\"><strong><font color=\"DarkRed\">" . $lang_functions['text_type'] . ": </font></strong>" . $type . "</td>
												" . ($media ? "<td class=\"clear\"><strong><font color=\"DarkRed\">" . $lang_functions['text_media'] . ": </font></strong>" . $media . "</td>" : "<td class=\"clear\"></td>") . "</tr>
												<tr><td class=\"clear\">" . "<strong><font color=\"DarkRed\">" . $lang_functions['text_singer'] . ": </font></strong>" . $singer . "</td>
												<td class=\"clear\"><strong><font color=\"DarkRed\">" . $lang_functions['text_tag'] . ": </font></strong>" . $tag . "</td></tr>
												<tr><td class=\"clear\"><strong><font color=\"DarkRed\">" . $lang_functions['text_publisher'] . ": </font></strong>" . $publisher . "</td>
												<td class=\"clear\"><strong><font color=\"DarkRed\">" . $lang_functions['text_disc'] . ": </font></strong>" . $disc . "</td></tr>" .
                ($summary ? "<tr><td class=\"clear\" colspan=\"2\">" . $summary . "</td></tr>" : "") . "
												</table>
												</td>
												</table>";
...
function get_torrent_promotion_append($promotion = 1, $forcemode = "", $showtimeleft = false, $added = "", $promotionTimeType = 0, $promotionUntil = '') {
  ...
  $timeout = gettime(date("Y-m-d H:i:s", $futuretime), false, false, true, false, true);
  if ($promotion != 1 && $promotionTimeType != 0) {
    $sp_torrent .= "&nbsp;[" . $lang_functions['text_will_end_in'] . "<b>" . $timeout . "</b>]";
  }
  return $sp_torrent;
}
...
function return_avatar_image($url) {
  global $CURLANGDIR;
  return "<img src=\"" . $url . "\" alt=\"avatar\" style=\"max-width: 150px; max-height: 250px;\" onload=\"check_avatar(this, '" . $CURLANGDIR . "');\" />";
}
