...
function format_comment($text, $strip_html = true, $xssclean = false, $newtab = false, $imageresizer = true, $image_max_width = 700, $enableimage = true, $enableflash = true , $imagenum = -1, $image_max_height = 0, $adid = 0)
{
    ...
    if ($adid) {
        ...
        $s = preg_replace("/\[url=([^\[\s]+?)\](.+?)\[\/url\]/ei", "formatUrl_s('\\1', ".($newtab==true ? 1 : 0).", '\\2', 'faqlink')", $s);
    }

    // [url]http://www.example.com[/url]
    $s = preg_replace("/\[url\]((https?|ftp|gopher|news|telnet|mms|rtsp):\/\/[^()\[\]<>\s]+)\[\/url\]/ei",
    "formatUrl('\\1', ".($newtab==true ? 1 : 0).", '', 'faqlink')", $s);
    $s = preg_replace("/\[url\]([^\[\s]+?)\[\/url\]/ei",
    "formatUrl_s('\\1', ".($newtab==true ? 1 : 0).", '', 'faqlink')", $s);
...
<table width="100%" cellspacing="0" cellpadding="5" border="0">
	<tr>
		<td align="left" colspan="2">
			<table cellspacing="1" cellpadding="2" border="0">
				<tr>
					<td class="embedded">
						<input style="font-weight: bold; font-size: 11px; margin-right: 3px" type="button" name="b" value="B" onclick="javascript: simpletag('b')" /></td>
					<td class="embedded">
						<input class="codebuttons" style="font-style: italic; font-size: 11px; margin-right: 3px" type="button" name="i" value="I" onclick="javascript: simpletag('i')" /></td>
					<td class="embedded">
						<input class="codebuttons" style="text-decoration: underline; font-size: 11px; margin-right: 3px" type="button" name="u" value="U" onclick="javascript: simpletag('u')" /></td>
					<?php
    ...
    print("<td class=\"embedded\"><input type=\"button\" style=\"font-size:11px;margin-right:3px\" name=\"list\" value=\"List\" onclick=\"tag_list('" . addslashes($lang_functions['js_prompt_enter_item']) . "','" . $lang_functions['js_prompt_error'] . "')\" /></td>");
					?>
					<td class="embedded">
						<input class="codebuttons" style="font-size: 11px; margin-right: 3px" type="button" name="quote" value="QUOTE" onclick="javascript: simpletag('quote')" /></td>
					<td class="embedded">
						<input style="font-size: 11px; margin-right: 3px" type="button" onclick='javascript:closeall();' name='tagcount' value="Close all tags" /></td>
					<td class="embedded">
...
function get_external_tr($url = "", $ext="imdb")
{
    ...
    if($showextinfo['imdb'] == 'yes' && $ext=="imdb" ) 
    {
        ...
        tr($lang_functions['row_imdb_url'],  "<input type=\"text\" style=\"width: 650px;\" name=\"url\" value=\"".($imdbNumber ? "http://www.imdb.com/title/tt".parse_imdb_id($url) : "")."\" /><br /><font class=\"medium\">".$lang_functions['text_imdb_url_note']."</font>", 1);
    }
...
{
    if (strpos($url,'/movie/subject/') != FALSE || strpos($url,'movie.douban.com') != FALSE)
    {
        return "movie";
    }
    else if(strpos($url,'/music/subject/') != FALSE || strpos($url,'music.douban.com') != FALSE)
    {
        return "music";
    }
    else
...
<body>
    ...
    else
    {
			...
			<td class="clear nowrap" align="right" valign="middle">
				...
				<a href="donate.php">
					<img src="<?= get_forum_pic_folder()?>/donate.gif" alt="Make a donation" style="margin-left: 5px; margin-top: 50px;" /></a>
				<?php
                      }
				?>
			</td>
	...
	<table class="mainouter" width="1060" cellspacing="0" cellpadding="5" align="center">
		<tr>
			<td id="nav_block" class="text" align="center">
				<?php if (!$CURUSER) { ?>
				<a href="login.php"><font class="big"><b><?= $lang_functions['text_login'] ?></b></font></a>/ <a href="signup.php"><font class="big"><b><?= $lang_functions['text_signup'] ?></b></font></a>
				<?php } 
                      else {
                          begin_main_frame();
                          menu ();
...
function deletetorrent($id, $deletesubs = FALSE /* 后向兼容 */) {
    ...
    if($deletesubs) {
        ...
        $sql = new_mysqli();
        $query = $sql->prepare("SELECT `id`, `ext`, `uppedby`, `title`, `anonymous`, `lang_name` FROM `subs` JOIN `language` ON `language`.`id` = `subs`.`lang_id` WHERE `torrent_id` = ?");
        ...
        while($query->fetch()) {
            ...
            // 插入删除字幕消息
            $msg = MessageFormatter::formatMessage($user_lang, $lang_upper['msg_delete_sub_format'], array($CURUSER['username'], $lang_upper['msg_reason_torrent_deleted']));
