...
if ($action == 'savesettings_main')	// save main
{
	...
	$validConfig = array('site_online','max_torrent_size','announce_interval', 'annintertwoage', 'annintertwo', 'anninterthreeage', 'anninterthree', 'signup_timeout','minoffervotes','offervotetimeout','offeruptimeout','maxsubsize','postsperpage', 'topicsperpage', 'torrentsperpage', 'maxnewsnum','max_dead_torrent_time','maxusers','torrent_dir', 'iniupload','SITEEMAIL', 'ACCOUNTANTID', 'ALIPAYACCOUNT', 'PAYPALACCOUNT', 'SLOGAN', 'icplicense', 'autoclean_interval_one', 'autoclean_interval_two', 'autoclean_interval_three','autoclean_interval_four', 'autoclean_interval_five','reportemail','invitesystem','registration','showhotmovies','showclassicmovies','showimdbinfo','showdoubaninfo', 'enablenfo', 'enableschool','restrictemail','showpolls','showstats','showlastxtorrents', 'showtrackerload','showshoutbox','showfunbox','showoffer','sptime','showhelpbox','enablebitbucket', 'smalldescription','altname','extforum','extforumurl','defaultlang','defstylesheet', 'donation','spsct','browsecat','specialcat','waitsystem','maxdlsystem','bitbucket','torrentnameprefix', 'showforumstats','verification','invite_count','invite_timeout', 'seeding_leeching_time_calc_start','startsubid', 'logo');
	GetVar($validConfig);
	unset($MAIN);
...
elseif ($action == 'savesettings_smtp') 	// save smtp
{
	...
	if ($smtptype == 'advanced') {
		$validConfig = array_merge($validConfig, array('smtp_host','smtp_port','smtp_from','smtpaddress','smtpport','smtpfrom','accountname','accountpassword','eYouAccount','eYouAccountDomain','eYouAccountPass','eYouUrl'));
	} elseif ($smtptype == 'external') {
		$validConfig = array_merge($validConfig, array('smtp_host','smtp_port','smtp_from','smtpaddress','smtpport','smtpfrom','accountname','accountpassword','eYouAccount','eYouAccountDomain','eYouAccountPass','eYouUrl'));
	} elseif ($smtptype == 'eYou') {
		$validConfig = array_merge($validConfig, array('smtp_host','smtp_port','smtp_from','smtpaddress','smtpport','smtpfrom','accountname','accountpassword','eYouAccount','eYouAccountDomain','eYouAccountPass','eYouUrl'));
	} elseif ($smtptype == 'none') {
		$validConfig = array_merge($validConfig, array('smtp_host','smtp_port','smtp_from','smtpaddress','smtpport','smtpfrom','accountname','accountpassword','eYouAccount','eYouAccountDomain','eYouAccountPass','eYouUrl'));
	}
...
elseif ($action == 'smtpsettings')	// stmp settings
{
	...
	$smtp_select = "<input type=\"radio\" name=\"smtptype\" value=\"default\" onclick=\"document.getElementById('smtp_advanced').style.display='none'; document.getElementById('smtp_external').style.display='none'; document.getElementById('smtp_eYou').style.display='none';\"".($SMTP['smtptype'] == "default" ? " checked" : "")."> ". $lang_settings['text_smtp_default'] .
	"<br /><input type=\"radio\" name=\"smtptype\" value=\"advanced\" onclick=\"document.getElementById('smtp_advanced').style.display=''; document.getElementById('smtp_external').style.display='none'; document.getElementById('smtp_eYou').style.display='none';\"".($SMTP['smtptype'] == "advanced" ? " checked" : "")."> " . $lang_settings['text_smtp_advanced'].
	"<br /><input type=\"radio\" name=\"smtptype\" value=\"external\" onclick=\"document.getElementById('smtp_advanced').style.display='none'; document.getElementById('smtp_external').style.display=''; document.getElementById('smtp_eYou').style.display='none';\"".($SMTP['smtptype'] == "external" ? " checked" : "")."> " . $lang_settings['text_smtp_external'].
	"<br /><input type=\"radio\" name=\"smtptype\" value=\"eYou\" onclick=\"document.getElementById('smtp_advanced').style.display='none'; document.getElementById('smtp_external').style.display='none'; document.getElementById('smtp_eYou').style.display='';\"".($SMTP['smtptype'] == "eYou" ? " checked" : "")."> " . $lang_settings['text_smtp_eYou'].
	"<br /><input type=\"radio\" name=\"smtptype\" value=\"none\" onclick=\"document.getElementById('smtp_advanced').style.display='none'; document.getElementById('smtp_external').style.display='none'; document.getElementById('smtp_eYou').style.display='none';\"".($SMTP['smtptype'] == "none" ? " checked" : "")."> " . $lang_settings['text_smtp_none'];
	...
	tr($lang_settings['row_outgoing_mail_from'], "<input type=text name=smtpfrom style=\"width: 300px\" ".($SMTP['smtpfrom'] ? "value=\"".$SMTP['smtpfrom']."\"" : "")."> ".$lang_settings['text_outgoing_mail_from_note'], 1);
	...
	print("</tbody><tbody id=\"smtp_eYou\"".($SMTP['smtptype'] == "eYou" ? "" : " style=\"display: none;\"").">");
	print("<tr><td colspan=2 align=center><b>".$lang_settings['text_setting_for_eYou_type']."</b></td></tr>");
	tr($lang_settings['row_eYouAccount'], "<input type=text name=eYouAccount style=\"width: 300px\" ".($SMTP['eYouAccount'] ? "value=\"".$SMTP['eYouAccount']."\"" : "")."> ".$lang_settings['text_eYouAccount_note'], 1);
	tr($lang_settings['row_eYouAccountDomain'], "<input type=text name=eYouAccountDomain style=\"width: 300px\" ".($SMTP['eYouAccountDomain'] ? "value=\"".$SMTP['eYouAccountDomain']."\"" : "")."> ".$lang_settings['text_eYouAccountDomain_note'], 1);
	tr($lang_settings['row_eYouAccountPass'], "<input type=password name=eYouAccountPass style=\"width: 300px\" ".($SMTP['eYouAccountPass'] ? "value=\"".$SMTP['eYouAccountPass']."\"" : "")."> ".$lang_settings['text_eYouAccountPass_note'], 1);
	tr($lang_settings['row_eYouUrl'], "<input type=text name=eYouUrl style=\"width: 300px\" ".($SMTP['eYouUrl'] ? "value=\"".$SMTP['eYouUrl']."\"" : "")."> ".$lang_settings['text_eYouUrl_note'], 1);
...
elseif ($action == 'securitysettings')	//security settings
{
	...
	tr($lang_settings['row_https_announce_url'],"<textarea type='text' rows=\"3\" style=\"width: 300px\" name=\"https_announce_url\">".($SECURITY["https_announce_url"] ? $SECURITY["https_announce_url"] : "")."</textarea> ".$lang_settings['text_https_announce_url_note'] . $_SERVER["HTTP_HOST"]."/announce.php", 1);
...
elseif ($action == 'authoritysettings')	//Authority settings
{
	...
	tr($lang_settings['row_enter_casino'], $lang_settings['text_minimum_class'].classlist('enter_casino',$maxclass,$AUTHORITY['enter_casino']).$lang_settings['text_default'].get_user_class_name(UC_POWER_USER,false,true,true).$lang_settings['text_enter_casino_note'],1);
    tr($lang_settings['row_view_bbcode'], $lang_settings['text_minimum_class'].classlist('view_bbcode',$maxclass,$AUTHORITY['view_bbcode']).$lang_settings['text_default'].get_user_class_name(UC_POWER_USER,false,true,true).$lang_settings['text_view_bbcode'],1);
	tr($lang_settings['row_save_settings'],"<input type='submit' name='save' value='".$lang_settings['submit_save_settings']."'>", 1);
	print ("</form>");
}
elseif ($action == 'basicsettings')	// basic settings
{
	...
	tr($lang_settings['row_announce_url'],"<textarea rows=\"3\" style=\"width: 300px\" name=\"announce_url\">".($BASIC["announce_url"] ? $BASIC["announce_url"] : $_SERVER["HTTP_HOST"]."/announce.php")."</textarea>".$lang_settings['text_it_should_be'] . $_SERVER["HTTP_HOST"]."/announce.php", 1);
...
elseif ($action == 'mainsettings')	// main settings
{
	...
	yesorno($lang_settings['row_enable_douban_system'],'showdoubaninfo', $MAIN['showdoubaninfo'], $lang_settings['text_douban_system_note']);
