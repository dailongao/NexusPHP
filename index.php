	...
	foreach($type as $type_each)
	{
		if($showmovies[$type_each] == 'yes' && (!isset($CURUSER) || $CURUSER['show' . $type_each] == 'yes'))
		{
			...
			if (!$Cache->get_page())
			{
				...
				if($type_each == 'hot')
					$res = sql_query("SELECT * FROM torrents WHERE picktype = " . sqlesc($type_each) . " AND seeders > 0 AND url != '' ORDER BY id DESC LIMIT  40") or sqlerr(__FILE__, __LINE__);
				else if ($type_each == 'classic')
					$res = sql_query("SELECT * FROM torrents WHERE picktype = " . sqlesc($type_each) . " AND seeders > 0 AND url != '' ORDER BY RAND() LIMIT  40") or sqlerr(__FILE__, __LINE__);
				if (mysql_num_rows($res) > 0)
				{
					$movies_list = "";
					$count = 0;
...
<table width="100%"><tr><td class="text"><div>
  <?php echo $lang_index['text_disclaimer_content'] ?></div></td></tr></table>
<?php
// ------------- end: disclaimer ------------------//
// ------------- start: links ------------------//
	...
	if (mysql_num_rows($res) > 0)
	{
		...
		print("<table width=\"100%\"><tr><td class=\"text\"><div>".trim($links)."</div></td></tr></table>");
	}
	$Cache->end_whole_row();
	$Cache->cache_page();
	}
