	...
	case "dailylog":
		...
		//echo $pagertop;
			...
			while ($arr = mysql_fetch_assoc($res))
			{
				...
				print("<tr><td class=\"rowfollow nowrap\" align=center>".gettime($arr['added'],true,false)."</td><td class=rowfollow align=left><div><font color='".$color."'>".htmlspecialchars($arr['txt'])."</font></div></td></tr>\n");
			}
			print("</table>");
	...
	case "chronicle":
		...
		//echo $pagertop;
			...
			while ($arr = mysql_fetch_assoc($res))
			{
				...
				print("<tr><td class=rowfollow align=center><nobr>$date</nobr></td><td class=rowfollow align=left><div>".format_comment($arr["txt"],true,false,true)."</div></td>".(get_user_class() >= $chrmanage_class ? "<td align=center nowrap><b><a href=\"".$PHP_SELF."?action=chronicle&do=edit&id=".$arr["id"]."\">".$lang_log['text_edit']."</a>&nbsp;|&nbsp;<a href=\"".$PHP_SELF."?action=chronicle&do=del&id=".$arr["id"]."\"><font color=red>".$lang_log['text_delete']."</font></a></b></td>" : "")."</tr>\n");
			}
			print("</table>");
			echo $pagerbottom;
		}
	...
	case "funbox":
		...
		//echo $pagertop;
			while ($arr = mysql_fetch_assoc($res)){
				$date = gettime($arr['added'],true,false);
			print("<table width=940 border=1 cellspacing=0 cellpadding=5>\n");
			print("<tr><td class=rowhead width='10%'>".$lang_log['col_title']."</td><td class=rowfollow align=left><div>".$arr["title"]." - <b>".$arr["status"]."</b></div></td></tr><tr><td class=rowhead width='10%'>".$lang_log['col_date']."</td><td class=rowfollow align=left><div>".$date."</div></td></tr><tr><td class=rowhead width='10%'>".$lang_log['col_body']."</td><td class=rowfollow align=left>".format_comment($arr["body"],false,false,true)."</td></tr>\n");
			print("</table><br />");
			}
			echo $pagerbottom;
		}
	...
	case "news":
		...
		//echo $pagertop;
			while ($arr = mysql_fetch_assoc($res)){
				$date = gettime($arr['added'],true,false);
			print("<table width=940 border=1 cellspacing=0 cellpadding=5>\n");
			print("<tr><td class=rowhead width='10%'>".$lang_log['col_title']."</td><td class=rowfollow align=left><div>".$arr["title"]."</div></td></tr><tr><td class=rowhead width='10%'>".$lang_log['col_date']."</td><td class=rowfollow align=left><div>".$date."</div></td></tr><tr><td class=rowhead width='10%'>".$lang_log['col_body']."</td><td class=rowfollow align=left><div>".format_comment($arr["body"],false,false,true)."</div></td></tr>\n");
			print("</table><br />");
			}
			echo $pagerbottom;
		}
