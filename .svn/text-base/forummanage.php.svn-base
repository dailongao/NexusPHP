...
elseif ($_POST['action'] == "editforum") {
	...
	sql_query("UPDATE forums SET sort = '" . $_POST['sort'] . "', name = " . sqlesc($_POST['name']). ", description = " . sqlesc($_POST['desc']). ", forid = ".sqlesc(($_POST['overforums'])).", minclassread = '" . $_POST['readclass'] . "', minclasswrite = '" . $_POST['writeclass'] . "', minclasscreate = '" . $_POST['createclass'] . "', casinomode = '" . $casinomode . "', casinoclass = '" . $_POST['casinoclass'] . "' where id = ".sqlesc($id)) or sqlerr(__FILE__, __LINE__);
	$Cache->delete_value('forums_list');
	$Cache->delete_value('forum_moderator_array');
	header("Location: forummanage.php");
	die();
...
elseif ($_POST['action'] == "addforum") {
	...
	sql_query("INSERT INTO forums (sort, name,  description, minclassread,  minclasswrite, minclasscreate, forid, casinomode, casinoclass) VALUES(" . $_POST['sort'] . ", " . sqlesc($_POST['name']). ", " . sqlesc($_POST['desc']). ", " . $_POST['readclass'] . ", " . $_POST['writeclass'] . ", " . $_POST['createclass'] . ", ".sqlesc(($_POST['overforums'])) . ", " . $casinomode . ", " . $_POST['casinoclass'] . ")") or sqlerr(__FILE__, __LINE__);
	$Cache->delete_value('forums_list');
	if ($_POST["moderator"]){
	$id = mysql_insert_id();
	$moderator = $_POST["moderator"];
...
?>
  ...
  <tr>
  <td><b><?php echo $lang_forummanage['col_forum_casinomode']?></b></td>
  <td>
  <input type="checkbox" name="casinomode" <?php
	if($row['casinomode'] == 1) echo "checked=checked";
  ?> >
  </td>
  </tr>
  
  <tr>
  <td><b><?php echo $lang_forummanage['col_forum_casinoclass']?></b></td>
  <td>
    <select name=casinoclass>
<?php
             $maxclass = get_user_class();
          for ($i = 0; $i <= $maxclass; ++$i)
            print("<option value=$i" . ($row["casinoclass"] == $i ? " selected" : "") . ">$prefix" . get_user_class_name($i,false,true,true));
?>
        </select>
  </td>
  </tr>
...
?>
  ...
  <td><b><?php echo $lang_forummanage['col_forum_casinomode']?></b></td>
  <td>
  <input type="checkbox" name="casinomode">
  </td>
  </tr>
