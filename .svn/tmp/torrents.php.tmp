...
if (!in_array($special_state,array(0,1,2,3)))
{
	$pick_type = 0;
	//write_log("User " . $CURUSER["username"] . "," . $CURUSER["ip"] . " is hacking pktype field in " . $_SERVER['SCRIPT_NAME'], 'mod');
}
if($pick_type == 0)	//all
{
	$addparam .= "pktype=0&";
}
elseif ($pick_type == 1)	//hot
{
	$addparam .= "pktype=1&";
	$wherea[] = "picktype = 'hot'";

}
elseif ($pick_type == 2)	//classic
{
	$addparam .= "pktype=2&";
	$wherea[] = "picktype = 'classic'";
}
elseif ($pick_type == 3)	//recommended
{
	$addparam .= "pktype=3&";
...
print("<table width=\"1024\" class=\"main\" border=\"0\" cellspacing=\"0\" cellpadding=\"0\"><tr><td class=\"embedded\">");
