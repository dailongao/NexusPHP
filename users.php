...
if (get_user_class() >= UC_ADMINISTRATOR && $email != '')
{
	$query = "email LIKE ". sqlesc("%$email%") . " AND status='confirmed'";
	$q .= ($q ? "&" : "") . "email=$email";
}
...
{
	print("<form method=get action=?>\n");
	print($lang_users['email_search'] ." <input type=text style=\"width:100px\" name=email value=$search> \n");
	print("<input type=submit value=\"".$lang_users['submit_okay']."\">\n");
	print("</form>\n");
}
...
print("<tr><td class=colhead align=left>".$lang_users['col_user_name']."</td>");
if (get_user_class() >= UC_ADMINISTRATOR) 
{
	print "<td class=colhead>".$lang_users['col_email']."</td>";
}
print("<td class=colhead>".$lang_users['col_registered']."</td><td class=colhead>".$lang_users['col_last_access']."</td><td class=colhead align=left>".$lang_users['col_class']."</td><td class=colhead>".$lang_users['col_country']."</td></tr>\n");
for ($i = 0; $i < $num; ++$i)
{
...
print ("<td>".gettime($arr['added'], true, false)."</td><td>".gettime($arr['last_access'],true,false)."</td><td align=left>". get_user_class_name($arr['class'],false,true,true) . "</td><td align=center>".$arr['country']."</td></tr>");
}
