...
while ($arr = mysql_fetch_assoc($res))
{
 ...
 <td class=embedded> ".($arr['showonline'] == 'yes'  ? (strtotime($arr['last_access']) > $dt ? $onlineimg : $offlineimg) : $lang_staff['text_hidden_by_staff'])."</td>".
...
unset($ppl);
$res = sql_query("SELECT forummods.userid AS userid, users.last_access, users.country, users.showonline FROM forummods LEFT JOIN users ON forummods.userid = users.id GROUP BY userid ORDER BY forummods.forumid, forummods.userid") or sqlerr();
while ($arr = mysql_fetch_assoc($res))
{
	$countryrow = get_country_row($arr['country']);
	$forums = "";
