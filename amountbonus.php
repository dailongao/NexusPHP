...
if ($_SERVER["REQUEST_METHOD"] == "POST")
{
	...
	// write log
	$res = sql_query("SELECT id, bonuscomment FROM users WHERE username=" . $username);
	$arr = mysql_fetch_assoc($res);
	$userid = $arr['id'];
	$bonuscomment = $arr['bonuscomment'];
	
	$points= number_format($seedbonus,1);
	if($seedbonus > 0)
	{
		$sign = " +";
	}
	...
	$bonuscomment = date("Y-m-d") .$sign.$points. " Points by staff ".($CURUSER["username"]).".\n " .htmlspecialchars($bonuscomment);
