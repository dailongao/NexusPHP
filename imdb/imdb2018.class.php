...
class Imdb
{
  ...
  public function getMovieInfoById($imdbId, $getExtraInfo = true)
  {
    ...
    $cache_file_path = __DIR__ . "/newcache/" . $imdbId;
    $cache_file = file_get_contents($cache_file_path);
    if ($cache_file === FALSE)
    {
        $result = $this->scrapeMovieInfo($imdbUrl, $getExtraInfo);
        file_put_contents($cache_file_path, json_encode($result));
        return $result;
    } else {
  ...
  private function geturl($url){
    ...
    curl_setopt($ch, CURLOPT_PROXY, "http://yq2.cc98.fun");
    curl_setopt($ch, CURLOPT_PROXYPORT, 808);
    curl_setopt($ch, CURLOPT_PROXYUSERPWD, "nhd:nhd");
