<?php
 ...
 class imdb extends imdb_config {
   ...
   *
   *	this function wastes too much net bandwith, so disable it temporarily.
   *	by stan
   */
	function similiar_movies()
	{
		...
		if (!isset($this->similiar_movies))
		{
			...
			$similiar_movies = $this->info_excer->truncate($this->page["Title"], "<h2>Recommendations</h2>", "<div class=\"see-more\">");
			$res_where_array = array('Link' => '1', 'Name' => '3');
			
			if($res_array = $this->info_excer->find_pattern($similiar_movies,"/<a\s+onclick=\".+\"\s+href=\"((\s|.)+?)\"\s+>((\s|.)+?)<\/a>/",true,$res_where_array))
			{
				$counter = 0;
				foreach($res_array as $res_array_each)
				{
					...
					$this->similiar_movies[$tmp]['Link'] = $res_array_each[0];
					$this->similiar_movies[$tmp]['Name'] = $res_array_each[1];
					...
					$this->similiar_movies[$tmp]['Local'] = $imdb_sim_movies->photo_localurl();
					$counter++;
				}
			}
...
$responseBody = $response->getBody();
  ...
  function runtime_all () {
   if ($this->main_runtime == "") {
    ...
    $this->main_runtime = trim(strip_tags(substr ($this->page["Title"], $runtime_s + 13, $runtime_e - $runtime_s - 14)));
    if ($runtime_s == 0)
	$this->main_runtime = "";
    }
...
#echo $this->runtime_all();
  ...
  function rating () {
   if ($this->main_rating == "") {
    ...
    $this->main_rating = substr ($this->page["Title"], $rate_s, $rate_e - $rate_s );
    if ($rate_e - $rate_s > 9) $this->main_rating = "";
  ...
  function comment () {
     if ($this->main_comment == "") {
      ...
      $comment_s = strpos ($this->page["Title"], "<h2>User Reviews</h2>");
  ...
  function genre () {
   if ($this->main_genre == "") {
    ...
    $this->main_genre = substr ($this->page["Title"], $genre_s + 2, $genre_e - $genre_s - 2);
   }
  ...
  function genres () {
   if ($this->main_genres == "") {
    ...
    $genre_e = strpos($this->page["Title"],"</div>",$genre_s);
  ...
  function tagline () {
   if ($this->main_tagline == "") {
    ...
    $block = trim(substr ($this->page["Title"], $tag_s, $tag_e - $tag_s - 1));
	$this->main_tagline = $block;
   }
...
#    $tag_s = strpos ($this->page["Title"], "http://ia.imdb.com/media",$tag_s);
  ...
  function alsoknow () {
   if ($this->main_alsoknow == "") {
	...
	$block = trim(substr($this->page["Title"],$ak_s,$alsoknow_end-$ak_s));
	$this->main_alsoknow = $block;
	/*
